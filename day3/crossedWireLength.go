...
func main() {
  ...
  for _, overlap := range overlaps {
    // get distance from both maps
    path1Length := path1[overlap]
    path2Length := path2[overlap]
    totalPath := path1Length + path2Length
    if totalPath < minDistance {
      minDistance = totalPath
    }
  }
  fmt.Printf("minDistance: %v\n", minDistance)
}

func getWirePath(wire []Path) map[string]int {
